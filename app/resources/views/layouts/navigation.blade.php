    ...
    <select name="locale" onchange="this.form.submit()" class="px-2 py-1 text-sm bg-white border rounded dark:bg-gray-900 dark:text-white">
        ...
        <option value="ru" {{ app()->getLocale() === 'ru' ? 'selected' : '' }}>RU</option>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @else
                    <span>{{ __('navigation.login_prompt') }}</span>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-600">
            <form action="{{ route('locale.change') }}" method="POST">
                @csrf
                <label for="mobile-locale" class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ __('navigation.language') }}
                </label>
                <select id="mobile-locale" name="locale" onchange="this.form.submit()" class="block w-full px-2 py-1 text-sm bg-white border rounded dark:bg-gray-900 dark:text-white">
                    <option value="en" {{ app()->getLocale() === 'en' ? 'selected' : '' }}>EN</option>
                    <option value="ru" {{ app()->getLocale() === 'ru' ? 'selected' : '' }}>RU</option>
                </select>
            </form>
